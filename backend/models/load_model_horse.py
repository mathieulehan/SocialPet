...
import cv2
import numpy as np
# loads models if not loaded already
...
def load_ground_up_model(weights=None):
    '''
    This function loads my custom models trained on augmented input data
    The models has a base of bottom-only xception, plus a custom dense network output
    Inputs:
        weights (String): filepath and filename of last .hdf5 file checkpoint. Default None
    Returns:
        ground_up_model (keras.Sequential): A models for classifying dog breeds
    '''
    ...
    print('assembling composite models...')
    ...
    print('compiling composite models...')
...
def final_predict_xception(final_model, img_path, topk=3):
    ...
    # load list of horse names
    horse_names = utils.loadRaceHorse()
    classes = [horse_names[x] for x in classes_ind][:topk]
...
def getHorseBreed(img):
    if 'ground_up_model' not in locals():
        ground_up_model = load_ground_up_model('./models/horse_breed_model.h5')

    final_classe = final_predict_xception(ground_up_model, img, topk=5)
